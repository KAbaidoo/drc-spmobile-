...
class HouseholdDetailFragment : BaseFragment<FragmentHouseholdDetailBinding>(FragmentHouseholdDetailBinding::inflate){
    val viewModel: HouseholdDetailViewModel by viewModel()
    var isOpen: Boolean = false

    val args: HouseholdDetailFragmentArgs by navArgs()
    override fun FragmentHouseholdDetailBinding.initialize() {
...
//            tvStatus.append(": ${if (household.status.isNullOrEmpty()) "not submitted" else household.status}")
   ...
   fun goToDeleteHouseholdDialog() {
        val bundle = bundleOf("id" to viewModel.id)
        navigateTo(R.id.deleteHouseholdDialogFragment, bundle)
    }
}
